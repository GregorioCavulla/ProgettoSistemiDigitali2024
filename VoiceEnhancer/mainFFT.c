...
void applyHardFilter(fftw_complex *fft, int N, double lowCut, double highCut, int sampleRate) {
    for (int k = 0; k < N; k++) {
        double frequency = (double)k * sampleRate / N; // Calcolo della frequenza
        ...
        if (frequency > 1800.0) {
            ...
            if(frequency >= lowCut && frequency <= highCut) {
                ...
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if(frequency >= lowCut + 200 && frequency <= highCut + 200) {
                // amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if(frequency >= lowCut + 400 && frequency <= highCut + 400) {
                // amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 600 && frequency <= highCut + 600){
                //amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 800 && frequency <= highCut + 800){
                //apmlifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 1000 && frequency <= highCut + 1000){
                //amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 1200 && frequency <= highCut + 1200){
                //amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 1400 && frequency <= highCut + 1400){
                //amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 1600 && frequency <= highCut + 1600){
                //amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else if (frequency >= lowCut + 1800 && frequency <= highCut + 1800){
                //amplifica le frequenze nella banda
                fft[k][0] *= 1.5;
                fft[k][1] *= 1.5;
            } else {
...
int main(int argc, char *argv[]) {
    ...
    double lowCut = 200.0;
    double highCut = 300.0;

    // Misura tempo filtro
    printf("Applicazione filtro...\n");
    clock_t startFilter = clock();
    applyHardFilter(fft, numSamples, lowCut, highCut, header.sampleRate);
