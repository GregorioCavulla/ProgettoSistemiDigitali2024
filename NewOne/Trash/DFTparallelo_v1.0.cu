...
void filtroHost(double *X_real, int N, int fc) {
    for (int k = 0; k < N; k++) {
        if (k > fc && k < N - fc) {
            X_real[k] = 0.0f;
        }
...
int main(int argc, char *argv[]) {
    ...
    snprintf(outputFile, sizeof(outputFile), "./output/cuda_v1.0_output_%s.wav", timestamp);
    snprintf(reportFile, sizeof(reportFile), "./reports/cuda_v1.0_report_%s.txt", timestamp);
