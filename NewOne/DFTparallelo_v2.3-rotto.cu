//Filtro kernel ottimizzato con float e ottimizzazioni basate sul report
...
// Kernel per calcolare la DFT (ottimizzato)
...
// Kernel per calcolare la IDFT (ottimizzato)
__global__ void idftKernel(const float *X_real, float *x, int N) {
    extern __shared__ float shared_X_real[];
...
int main(int argc, char *argv[]) {
    ...
    snprintf(outputFile, sizeof(outputFile), "./output/cuda_v2.3_output_%s.wav", timestamp);
    snprintf(reportFile, sizeof(reportFile), "./reports/cuda_v2.3_report_%s.txt", timestamp);
