...
void aggiornaVelocità(int input, Vettore vettori[], int num_vettori) {
    // Funzione per aggiornare la velocità dei vettori in base all'input
    for (int i = 0; i < num_vettori; i++) {
        switch (input) {
            case 2:  // Diminuisce v di 0.2
                vettori[i].v -= 1;
                break;
            case 4:  // Aumenta v di 0.2
                vettori[i].u -= 1;
                break;
            case 6:  // Diminuisce u di 0.2
                vettori[i].u += 1;
                break;
            case 8:  // Aumenta u di 0.2
                vettori[i].v += 1;
                break;
            default:
                printf("Input non valido! Devi inserire 2, 4, 6, o 8.\n");
                break;
        }
    }
}
...
int main() {
    ...
    for (int i = 0; i < NUM_VETTORI; i++) {
        ...
        // Velocità iniziale
        vettori[i].u = 1.0;  // Velocità iniziale lungo l'asse X
    ...
    for (int t = 0; t < DURATA; t++) {
        ...
        int input;
        printf("Inserisci un numero (2, 4, 6, 8): ");
        scanf("%d", &input);

        // Aggiorna la velocità dei vettori in base all'input dell'utente
        aggiornaVelocità(input, vettori, NUM_VETTORI);
