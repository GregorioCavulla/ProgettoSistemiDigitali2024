...
int main() {
    ...
    // Velocità variabile per ciascun vettore (assegnato casualmente per esempio tra 1 e 6)
    double vettore_velocità[NUM_VETTORI];
    for (int i = 0; i < NUM_VETTORI; i++) {
        // Assegna una velocità casuale tra 1 e 6 per ogni vettore
        vettore_velocità[i] = (rand() % 6) + 1;  // Numero casuale tra 1 e 6
    }
    ...
    for (int t = 0; t < DURATA; t++) {
        ...
        for (int i = 0; i < NUM_VETTORI; i++) {
            // Ogni vettore si sposta verso destra ogni secondo in base alla sua velocità
            x_pos[i] += vettore_velocità[i];  // Usa la velocità specifica del vettore
